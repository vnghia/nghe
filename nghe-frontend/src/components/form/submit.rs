        ...
        .class(move || {
            if action_pending() {
                "w-full text-white bg-primary-600 font-medium rounded-lg px-5 py-2.5 text-center \
                 dark:bg-primary-600 cursor-not-allowed"
            } else {
                "w-full text-white bg-primary-600 hover:bg-primary-700 focus:ring-4 \
                 focus:outline-none focus:ring-primary-300 font-medium rounded-lg px-5 py-2.5 \
            ...
            if action_pending() {
                Either::Left(
                    html::div().role("status").child((
                                ...
                                svg::path()
                                    .attr(
                                        ...
                                        "M100 50.5908C100 78.2051 77.6142 100.591 50 \
                                         ...
                                         100 50.5908ZM9.08144 50.5908C9.08144 73.1895 27.4013 \
                                         91.5094 50 91.5094C72.5987 91.5094 90.9186 73.1895 \
                                         90.9186 50.5908C90.9186 27.9921 72.5987 9.67226 50 \
                        ...
                        html::span().class("sr-only").child("Loading..."),
                    )),
                )
            } else {
                Either::Right(button)
