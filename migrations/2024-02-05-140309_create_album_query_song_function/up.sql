...
create function album_count_song_by_music_folder (_music_folder_ids uuid[], _album_id uuid) returns bigint language sql stable leakproof parallel safe return (
  select
    count(*)
  from
    album_query_song_by_music_folder (_music_folder_ids, _album_id)
);

create function album_count_song_by_user (_user_id uuid, _album_id uuid) returns bigint language sql stable leakproof parallel safe return (
  select
    count(*)
  from
    album_query_song_by_user (_user_id, _album_id)
);