...
songs (
    ...
    languages text [] not null default array[]::text [] check (
    ...
    -- Constraints
    constraint songs_album_id_fkey foreign key (album_id) references albums (
        id
    ) on delete cascade,
    constraint songs_music_folder_id_relative_path_key unique (
        music_folder_id, relative_path
    ),
    constraint songs_music_folder_id_fkey foreign key (
        music_folder_id
    ) references music_folders (id) on delete cascade,
    constraint songs_music_folder_id_file_hash_file_size_key unique (
        music_folder_id, file_hash, file_size
    )
);

select add_updated_at_leave_scanned_at('songs');
