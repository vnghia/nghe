...
pub async fn build_music_folder_permissions(pool: &DatabasePool) -> Result<Vec<Uuid>> {
    ...
    if !missing_folder_ids.is_empty() {
        ...
        set_music_folder_permissions(pool, &user_ids, &missing_folder_ids, true).await?;
    }
...
mod tests {
    ...
    async fn test_build_nothing() {
        ...
        let missing_folder_ids = build_music_folder_permissions(infra.pool()).await.unwrap();
    ...
    async fn test_build_missing() {
        ...
        // `build_music_folder_permissions` should not override exisiting permissions.
