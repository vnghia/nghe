...
) -> Result<Vec<(String, ArtistAlbumCountId3Db)>> {
    #[add_permission_filter]
    get_album_artist_id3_db()
        .select((artists::index, ArtistAlbumCountId3Db::as_select()))
        .get_results::<(String, ArtistAlbumCountId3Db)>(&mut pool.get().await?)
        .await
...
pub async fn get_artists(
    ...
    let index = get_indexed_artists(pool, user_id, music_folder_ids)
        ...
        .map(|(k, v)| Index {
            name: k,
            artists: v.into_iter().map(ArtistAlbumCountId3Db::into).collect(),
        })
        .collect_vec();
...
mod tests {
    ...
    async fn test_get_indexed_artists() {
        ...
        let artist_ids = get_indexed_artists(infra.pool(), infra.user_id(0), &None)
            ...
            .unwrap()
            .into_iter()
            .map(|(_, artist)| artist.artist.basic.id)
            .sorted()
            .collect_vec();
    ...
    async fn test_get_artists() {
        ...
        let mut infra = Infra::new().await.n_folder(n_folder).await.add_user(None).await;
        for i in 0..n_folder {
            infra.add_n_song(i, n_song).await;
        }
        infra.scan(.., None).await;
        infra.remove_permission(None, None).await.add_permissions(.., 0..2).await;
