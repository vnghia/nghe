...
pub async fn get_song_download_info(
    ...
    music_folders::table
        ...
        .select((music_folders::path, songs::relative_path, songs::file_size))
        .first::<(String, String, i64)>(&mut pool.get().await?)
        .await
        .optional()?
        .ok_or_else(|| OSError::NotFound("Song".into()).into())
        .map(|(music_folder_path, song_relative_path, song_file_size)| {
            (Path::new(&music_folder_path).join(song_relative_path), song_file_size as _)
        })
}
